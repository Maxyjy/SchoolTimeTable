package com.android.mjutimetable.spinner;
...
import com.android.mjutimetable.spinner.adapter.MySpinnerPopListArrayAdapter;
import com.android.mjutimetable.spinner.listener.OnSpinnerClickListener;
import com.android.mjutimetable.spinner.listener.OnSpinnerItemClickListener;
import com.android.mjutimetable.spinner.util.DialogUtil;
import com.android.mjutimetable.spinner.util.PopWindowUtil;
...
public class SpinnerViewPop extends RelativeLayout {
	...
	public SpinnerViewPop(Context context, AttributeSet attrs) {
		...
		textDefaultColor = getResources().getColor(R.color.white);
		//默认文本颜色
	...
	private void showSelectedState(boolean isSelected){
		...
		if(isSelected){
			//修改箭头图标
			Drawable right= ContextCompat.getDrawable(mContext,R.drawable.ic_expand_less_black_24dp);
            right.setBounds(0,0, PxUtil.dip2px(mContext,20),PxUtil.dip2px(mContext,20));

			titleTextView.setCompoundDrawables(null,null,right,null);
			...
			//修改箭头图标
			Drawable right= ContextCompat.getDrawable(mContext,R.drawable.ic_expand_more_black_24dp);
			right.setBounds(0,0, PxUtil.dip2px(mContext,20),PxUtil.dip2px(mContext,20));
