...
import com.android.mjutimetable.spinner.adapter.MySpinnerPopMultListArrayAdapter;
import com.android.mjutimetable.spinner.listener.OnSpinnerClickListener;
import com.android.mjutimetable.spinner.listener.OnSpinnerConfirmClickListener;
...
public class SpinnerViewMultiDialog extends RelativeLayout {
	...
	public void setHint(String hint) {
		...
		titleTextView.setHintTextColor(Color.BLACK);
	...
	private void showSelectedState(boolean isSelected){
		...
		if(isSelected){
			//修改箭头图标
			Drawable right= ContextCompat.getDrawable(mContext,R.drawable.spinnerview_pop_icon_shang);
			right.setBounds(0,0,30,20);//必须设置图片的大小否则没有作用
			titleTextView.setCompoundDrawables(null,null,right,null);
			//修改文本颜色
			titleTextView.setTextColor(getResources().getColor(R.color.white));
		}else{
			//修改箭头图标
			Drawable right= ContextCompat.getDrawable(mContext,R.drawable.spinnerview_pop_icon_xia);
			right.setBounds(0,0,30,20);//必须设置图片的大小否则没有作用
			titleTextView.setCompoundDrawables(null,null,right,null);
			//修改文本颜色
			titleTextView.setTextColor(getResources().getColor(R.color.white));
